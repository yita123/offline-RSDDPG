...
for t in range(num_steps + test_num_steps):
    ...
    for i in range(start_pos, stop_pos):
        ...
        if curr_his.size == num_his:
            ...
            if t >= num_steps:
                # testing sample
                test_buffer.put(curr_his.get_obs_history(), curr_his.get_action_history(), curr_his.get_reward_history())
            else:
                # training sample
                replay_buffer.put(curr_his.get_obs_history(), curr_his.get_action_history(), curr_his.get_reward_history())
        elif t == 0:
...
for episode in range(num_episodes):
    ...
    with tf.GradientTape() as tape:
        ...
        critic_loss = critic_loss_fun(tf.stop_gradient(target_values), Qpredicts)
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        actor_supervised_loss = actor_supervised_loss_func(actor_actions, action_history[:, 1:, :])
        predict_q_value = tf.math.reduce_mean(critic(obs_history[:, 1:, :], actor_actions))
        ...
        actor_supervised_test_loss = actor_supervised_loss_func(actor_test_pred_actions, action_test[:, 1:, :])
        if episode == num_episodes:
            ...
            actor_test_res_df.to_excel(r'C:\Users\Think\Desktop\RL\rf_learning\Data\raw_data_RL-test-rsrl-his.xlsx', index=None)
    ...
    print(f'critic_loss {critic_loss}: actor_supervised_loss = {actor_supervised_loss}: q_value = {predict_q_value} : actor_supervised_test_loss = {actor_supervised_test_loss}: actor_loss_weight = {actor_loss_weight}')
    train_his_info.append([episode, critic_loss.numpy(), actor_supervised_loss.numpy(), predict_q_value.numpy(), actor_supervised_test_loss.numpy()])

train_his_df = pd.DataFrame(train_his_info, columns=['episode', 'critic_loss', 'actor_supervised_loss', 'predict_q_value', 'actor_supervised_test_loss'])
train_his_df.to_excel(r'C:\Users\Think\Desktop\RL\rf_learning\Data\raw_data_RL-train-rsrl-his.xlsx', index=None)

# save model
actor.save(r'C:\Users\Think\Desktop\RL\rf_learning\models\rsrl\actor')
